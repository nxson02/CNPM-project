...
export default function HeaderLinks(props) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('bluemoon:notifications-updated', handleNotificationUpdate);
    return () => window.removeEventListener('bluemoon:notifications-updated', handleNotificationUpdate);
  }, [fetchNotifications]);
  ...
  const handleNotificationClick = async (notification, event) => {
    ...
    try {
      ...
      if (linkHeader && linkAPI) {
        ...
        setTimeout(() => {
          const target = linkAPI.includes('/bills/') || linkAPI.includes('apartment-detail')
            ? '/user/payment'
            : linkAPI;
          navigate(target);
